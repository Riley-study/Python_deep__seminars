...
class TestEmployee:
    def test_employee_full_name(self):
        ...
        assert self.p1.full_name() == 'Ivanov Ivan Ivanovich'

    def test_employee_birthday(self):
        e1 = Employee('Ivanov', 'Ivan', 'Ivanovich', 30, "manager", 50000)
        e1.birthday()
        assert e1.get_age() == 31

    def test_employee_raise_salary(self):
        e1 = Employee('Ivanov', 'Ivan', 'Ivanovich', 30, "manager", 50000)
        e1.raise_salary(10)
        assert int(e1.salary) == 55000

    def test_employee_str(self):
        e1 = Employee('Ivanov', 'Ivan', 'Ivanovich', 30, "manager", 50000)
        assert e1.__str__() == "Ivanov Ivan Ivanovich (Manager)"

    def test_employee_last_name_title(self):
        e1 = Employee('Ivanov', 'Ivan', 'Ivanovich', 30, "manager", 50000)
        assert e1.last_name == "Ivanov"
